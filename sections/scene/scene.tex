...
Additionally, we calculate \textbf{surrounding scene descriptors}. SIFT descriptors are extracted in a dense fashion, filtering out those regions related with the foreground motion. Due to the extensive computation demand, we use only a subset of the frames for scene feature extraction. Generally, we sample 10\% of the number of frames for this purpose. Finally, SIFT descriptors are computed from the feature points associated with those sampled frames.
