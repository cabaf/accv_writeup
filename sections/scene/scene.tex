...
Additionally, we calculate \textbf{surrounding scene descriptors}. SIFT descriptors are extracted in a dense fashion, filtering out those regions related with the foreground motion. Due to the extensive computation demand, we sample 10\% of the number of frames. Then, SIFT descriptors are computed from feature points associated with those sampled frames.
